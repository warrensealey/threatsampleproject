...
from backend.config import (
    ...
    save_config,
    get_smtp_config,
    get_email_client_config,
    get_email_generation_config,
    update_smtp_config,
    update_email_client_config,
    update_email_generation_config,
    get_history,
    get_all_email_client_configs,
    save_email_client_config,
    delete_email_client_config,
...
def list_templates():
    ...
    Query parameters:
        type: Optional template type (e.g. \"custom\", \"phishing\").
    """
    try:
...
def upsert_template():
    ...
    try:
        ...
        success = save_template(template_type, name, template_data)
        if not success:
            return jsonify({"error": "Failed to save template"}), 500

        return jsonify({"success": True, "type": template_type, "name": name})
    except Exception as e:
...
def delete_template_route(template_type, name):
    ...
    try:
        success = delete_template(template_type, name)
        if success:
            return jsonify({"success": True})
